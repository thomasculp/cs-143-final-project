...
marygao@college.harvard.edu}
\and
\IEEEauthorblockN{Thomas Culp}
culp@college.harvard.edu}
}
...
physical place in a room. The scenarios which $P_{ij}$ are
modeled after can be categorized as \textit{office hours, random clusters, and
future usage} (see \ref{grid}).
...
is the mean over the set of throughputs each device in $P_{ij}$ receives, and
...
all the other routers). Power is modeled by the Friis transmission equation,
...
\textbf{TODO}: write up the algorithm used, which may change a little with time.
